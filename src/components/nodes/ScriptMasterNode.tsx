...
const SCRIPT_MASTER_TIMELINE_RULER_HEIGHT = 28;
const SCRIPT_MASTER_TIMELINE_ROW_HEIGHT = 36;
const SCRIPT_MASTER_TIMELINE_VERTICAL_OVERSCAN = 6;
const SCRIPT_MASTER_ASSET_ROW_HEIGHT = 98;
...
const ScriptMasterNode = ({ id, data, selected }: NodeProps) => {
  ...
  const timelineLaneWidth = Math.max(1, timelineViewport.scrollWidth - 220);
  const timelineVisibleStartRatio = Math.max(0, timelineViewport.scrollLeft - 220) / timelineLaneWidth;
  const timelineVisibleEndRatio = Math.min(1, Math.max(0, timelineViewport.scrollLeft + timelineViewport.width - 220) / timelineLaneWidth);
  ...
  const workbench = workbenchOpen && typeof document !== 'undefined' ? createPortal(
    <div role="dialog" aria-modal="true" aria-label="剧本大师工作台" className="nodrag nowheel fixed inset-0 z-[2140] flex min-h-0 flex-col overflow-hidden bg-[#07090d] text-white" data-script-master-workbench="true">
      <header className="flex h-16 shrink-0 items-center gap-3 border-b border-white/10 bg-[#0a0d13] px-4 shadow-2xl">
        <div className="grid h-9 w-9 place-items-center rounded-xl border border-violet-300/30 bg-violet-300/10 text-violet-100"><BookOpenText size={18} /></div>
        ...
        <div className="relative ml-3">
          ...
          {globalSettingsOpen && <section className="absolute left-0 top-11 z-40 w-72 rounded-xl border border-white/10 bg-[#111620] p-3 shadow-2xl">
      ...
      <div className="grid min-h-0 flex-1 grid-rows-[minmax(0,1fr)_minmax(230px,37vh)]">
        <div className="grid min-h-0 grid-cols-[270px_minmax(0,1fr)_330px]">
          <aside className="min-h-0 overflow-y-auto border-r border-white/10 bg-[#0b0e14] p-3">
            <section className="mb-3 rounded-xl border border-white/10 bg-white/[0.025] p-3">
            ...
            <div className="mb-2 grid grid-cols-3 gap-1">
              <label className="flex cursor-pointer items-center justify-center gap-1 rounded-lg border border-white/10 px-1 py-2 text-[9px] text-white/60"><ImageIcon size={11} />图/视频<input multiple type="file" accept="image/*,video/*" className="hidden" onChange={handleAssetFiles} /></label>
              <label className="flex cursor-pointer items-center justify-center gap-1 rounded-lg border border-white/10 px-1 py-2 text-[9px] text-white/60"><AudioLines size={11} />音频<input multiple type="file" accept="audio/*" className="hidden" onChange={handleAssetFiles} /></label>
              <button disabled={uploading} onClick={syncUpstream} className="flex items-center justify-center gap-1 rounded-lg border border-cyan-300/20 px-1 py-2 text-[9px] text-cyan-100 disabled:opacity-40"><RefreshCw size={11} className={uploading ? 'animate-spin' : ''} />上游</button>
            </div>
            ...
            <div ref={assetScrollerRef} role="list" aria-label="剧本大师素材列表" aria-setsize={filteredAssets.length} onScroll={(event) => setAssetViewport({ scrollTop: event.currentTarget.scrollTop, height: event.currentTarget.clientHeight })} className="max-h-[min(42vh,430px)] overflow-y-auto pr-1">
              ...
              {visibleAssets.map((asset, visibleIndex) => {
                ...
                return <article key={asset.id} role="listitem" aria-posinset={assetStartIndex + visibleIndex + 1} aria-setsize={filteredAssets.length} className={`h-[90px] overflow-hidden rounded-xl border p-2 ${assetTone(asset)}`}>
                  <div className="flex gap-2">
                    <div className="grid h-12 w-14 shrink-0 place-items-center overflow-hidden rounded-lg border border-white/10 bg-black/30">
                  ...
                  {binding && <div className="mt-2 flex items-center gap-1.5"><select value={binding.scope} onChange={(event) => changeBindingScope(binding, event.target.value as ScriptMasterScope)} className="min-w-0 flex-1 rounded border border-white/10 bg-[#121722] px-2 py-1 text-[9px] text-white/65">{(Object.keys(SCOPE_LABELS) as ScriptMasterScope[]).map((scope) => <option key={scope} value={scope}>{SCOPE_LABELS[scope]}</option>)}</select><button onClick={() => commit(patchScriptMasterBinding(projectRef.current, binding.id, { locked: !binding.locked }))} className={binding.locked ? 'text-amber-200' : 'text-white/35'}>{binding.locked ? <Lock size={11} /> : <Unlock size={11} />}</button></div>}
                </article>;
              })}
              </div>
          ...
          <main className="relative min-h-0 overflow-y-auto bg-[radial-gradient(circle_at_50%_25%,rgba(79,70,229,0.12),transparent_42%)] p-4">
          ...
          <aside className="min-h-0 overflow-y-auto border-l border-white/10 bg-[#0b0e14] p-3">
        ...
        <section className="flex min-h-0 flex-col overflow-hidden border-t border-white/10 bg-[#090c12]">
          <div className="flex h-10 shrink-0 items-center gap-2 border-b border-white/10 px-3"><Scissors size={13} className="text-violet-200" /><strong className="text-[11px]">多轨时间线</strong><span className="text-[9px] text-white/30">整数帧 · [start,end) · 多参考/多音频独立轨</span><span className="rounded border border-white/10 px-1.5 py-0.5 text-[8px] text-white/35">已选 {selectedTimelineItems.length}</span><div className="ml-auto flex items-center gap-1"><button onClick={() => { const zoom = Math.max(0.5, projectRef.current.timeline.zoom - 0.25); setTimelineController((current) => applyVideoEditTimelineControllerCommand(current, { type: 'zoom', zoom }, scriptMasterControllerOptions(projectRef.current))); commit(patchScriptMasterTimelineView(projectRef.current, { zoom }), {}, { recordHistory: false, viewOnly: true }); }} className="rounded border border-white/10 p-1.5 text-white/45" title="缩小时间线"><ZoomOut size={11} /></button><span className="w-10 text-center text-[9px] text-white/35">{Math.round(project.timeline.zoom * 100)}%</span><button onClick={() => { const zoom = Math.min(4, projectRef.current.timeline.zoom + 0.25); setTimelineController((current) => applyVideoEditTimelineControllerCommand(current, { type: 'zoom', zoom }, scriptMasterControllerOptions(projectRef.current))); commit(patchScriptMasterTimelineView(projectRef.current, { zoom }), {}, { recordHistory: false, viewOnly: true }); }} className="rounded border border-white/10 p-1.5 text-white/45" title="放大时间线"><ZoomIn size={11} /></button></div></div>
          <div className="flex h-9 shrink-0 items-center gap-1 border-b border-white/10 bg-[#0a0e15] px-3">
            <button disabled={!historyRef.current.past.length} onClick={undoWorkbenchChange} className="rounded border border-white/10 p-1.5 text-white/45 disabled:opacity-20" title="撤销 Ctrl+Z"><RotateCcw size={11} /></button>
            <button disabled={!historyRef.current.future.length} onClick={redoWorkbenchChange} className="rounded border border-white/10 p-1.5 text-white/45 disabled:opacity-20" title="重做 Ctrl+Y"><RotateCw size={11} /></button>
            ...
            <span className="ml-auto text-[8px] text-white/25">拖片段移动 · 拖两端修剪 · Shift/Ctrl 多选 · 方向键 ±1 帧</span>
          </div>
          <div ref={timelineScrollerRef} role="grid" aria-label="剧本大师多轨时间线" aria-rowcount={project.timeline.tracks.length} onScroll={(event) => updateTimelineViewport(event.currentTarget)} onPointerDown={beginTimelineMarquee} className="relative min-h-0 flex-1 overflow-auto">
            <div className="relative" style={{ minWidth: `${Math.max(100, project.timeline.zoom * 100)}%` }}>
              ...
              <div className="sticky top-0 z-20 grid h-7 grid-cols-[220px_minmax(700px,1fr)] border-b border-white/10 bg-[#0b0f17]"><div className="border-r border-white/10 px-3 py-1 text-[9px] text-white/30">轨道组 / 子轨</div><div className="relative cursor-crosshair" onClick={handleTimelineClick}>{[0, 0.25, 0.5, 0.75, 1].map((ratio) => <span key={ratio} style={{ left: `${ratio * 100}%` }} className="absolute top-1 -translate-x-1/2 text-[8px] text-white/25">{formatScriptMasterTimecode(rationalTime(Math.round(project.targetDurationFrames * ratio), project.fps), project.fps)}</span>)}<span style={{ left: `${project.targetDurationFrames ? project.timeline.playhead.value / project.targetDurationFrames * 100 : 0}%` }} className="pointer-events-none absolute inset-y-0 z-30 w-px bg-rose-300" /></div></div>
              ...
              {visibleTimelineTracks.map((track, visibleTrackIndex) => {
                ...
                return <div key={track.id} role="row" aria-rowindex={timelineTrackStart + visibleTrackIndex + 1} className={`grid h-9 grid-cols-[220px_minmax(700px,1fr)] border-b border-white/[0.06] ${track.hidden ? 'opacity-55' : ''}`}>
                  <div role="rowheader" className="flex items-center gap-1.5 border-r border-white/10 px-3"><span className="w-8 rounded border border-white/10 px-1 py-0.5 text-center text-[8px] text-white/35">{GROUP_LABELS[track.group]}</span><span className="min-w-0 flex-1 truncate text-[9px] text-white/55">{track.name}</span><button aria-label={`${track.locked ? '解锁' : '锁定'}轨道 ${track.name}`} onClick={() => commit(patchScriptMasterTimelineTrack(projectRef.current, track.id, { locked: !track.locked }), {}, { historyKey: `track:${track.id}` })} className={track.locked ? 'text-amber-200' : 'text-white/25'} title={track.locked ? '解锁轨道' : '锁定轨道'}>{track.locked ? <Lock size={9} /> : <Unlock size={9} />}</button><button aria-label={`${track.hidden ? '显示' : '隐藏'}轨道 ${track.name}`} onClick={() => commit(patchScriptMasterTimelineTrack(projectRef.current, track.id, { hidden: !track.hidden }), {}, { historyKey: `track:${track.id}` })} className={track.hidden ? 'text-white/20' : 'text-white/35'} title={track.hidden ? '显示轨道' : '隐藏轨道'}>{track.hidden ? <EyeOff size={9} /> : <Eye size={9} />}</button><button aria-label={`${track.muted ? '取消静音' : '静音'}轨道 ${track.name}`} onClick={() => commit(patchScriptMasterTimelineTrack(projectRef.current, track.id, { muted: !track.muted }), {}, { historyKey: `track:${track.id}` })} className={track.muted ? 'text-rose-200/70' : 'text-white/35'} title={track.muted ? '取消静音' : '静音'}>{track.muted ? <VolumeX size={9} /> : <Volume2 size={9} />}</button></div>
                  <div className="relative min-h-9 cursor-crosshair bg-[linear-gradient(90deg,rgba(255,255,255,0.025)_1px,transparent_1px)] bg-[size:10%_100%]" onClick={handleTimelineClick}>
      ...
      <footer
        className="flex h-7 shrink-0 items-center gap-3 border-t border-white/10 bg-[#080a0f] px-3 text-[9px] text-white/35"
        role="status"
        aria-live="polite"
